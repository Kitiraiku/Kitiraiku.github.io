...
SPAWN_RULES.defaults.doSpawn = function(b){
    // tropical waves
    if(random()<(0.01*sq((seasonalSine(b.tick)+1.001)/2)+0.001)) b.spawnArchetype('tw');
    if(Math.round(random(1, 530)) == 1) b.spawnArchetype('tw');

    // extratropical cyclones
    if(random()<0.0052-0.002*seasonalSine(b.tick)) b.spawnArchetype('ex');
    if(Math.round(random(1, 500)) == 2) b.spawnArchetype('ex');
};
...
ENV_DEFS[SIM_MODE_WILD].ULSteering = {
    mapFunc: (u,x,y,z)=>{
        ...
        let fAngle = 5.3*PI/8;                                                            // angle of winds poleward of jetstream
...
ENV_DEFS.defaults.SST = {
    ...
    mapFunc: (u,x,y,z)=>{
        ...
        let w = map(cos(map(x,0,WIDTH,0,PI)),-1,1,0.775,1);
        let h0 = y/HEIGHT;
        let h1 = (sqrt(h0)+h0)/2;
        let h2 = sqrt(sqrt(h0));
    ...
    modifiers: {
        ...
        peakSeasonPolarTemp: 14.5,
        offSeasonTropicsTemp: 25.1,
        peakSeasonTropicsTemp: 28.05
    }
};
...
STORM_ALGORITHM.defaults.core = function(sys,u){
    ...
    )*map(jet*0.5+1.3*(0.05-0.1*random(0,1)),0,75,sq(1-sys.depth),1,true);
    ...
    sys.organization -= 1.43*map(moisture,0,0.68,3,0,true)*(shear*2.2);
    sys.organization += sq(map(moisture,0.6,1,0,1,true))*4.41;
    ...
    if((moisture < 0.38) && (random(1,60) < 3)) sys.organization -= sys.pressure > 990 ? random(2,4) : random(0,2); // Intenser degrade due to very lacking moisture
    if(sys.broadening && (sys.organization < 80)) sys.organization -= random(0,1) / 2; // Broad boy
    sys.organization -= (pow(1.82,shear*1.3))*(0.2-sys.organization/100);
    ...
    let targetPressure = 1010-25*log((lnd||SST<24)?1:map(SST,24,30.7-((shear>0.85+random(0,2)/10)?1-moisture:pow(1.3,1+Math.round(moisture*1000)/1000)),1,2))/log(1.16);
    ...
    // Additional Factors:
    if(moisture > 0.6) sys.pressure -= (random(0,3) / 6) * (nontropicalness + 1); // Non-SST related instabilty from moisture and extratropicalness
    if((sys.pressure < random(960,990)) && (random(0,500 - 3*round(100*(1 - moisture))) == 0)) sys.broadening = true; // EWRC
    if(sys.organization > 0.95) sys.pressure += (pow(1.41,1 + SST/9.5) - 1) / 2; // SST Impact Nerf
    ...
    if(sys.broadening && (sys.pressure < random(905,920))) sys.pressure += (random(0,2) - 0.9) / 1.3; // Imploding
    ...
    if(sys.broadening) sys.windSpeed -= random(0,2) / 2;
    
    if((random(0,50) == 50) && (moisture > 0.69) && ((SST > 25.9) && (sys.organization < 1))) sys.broadening = false;
    if((random(0,105) == 50) && (moisture > 0.69)) sys.broadening = false;
    ...
    )*0.6*(1.1*tropicalness+0.4*nontropicalness);
